...
parser.add_argument('--outf', default='./training_lr_0.0002', help='folder to output images and model checkpoints')
...
dataloader = utils.load_data(opt.batchSize, opt.dataset)
nc = 1
...
# setup optimizer
optimizerD = optim.Adam(netD.parameters(), lr=opt.lr/1.5, betas=(opt.beta1, 0.999))
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if i % 100 == 0:
            ...
            vutils.save_image(fake.detach(),
                              ...
                              normalize=True)
            data_plot = fake[0].squeeze().cpu().detach().numpy()
            np.save(opt.outf+'/{}.npy'.format(epoch), data_plot)
            f = plt.figure()
